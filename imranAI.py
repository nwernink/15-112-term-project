# Super Agressive/Imran poker AI
from montecarloodds import *
from pokerml import *

# runs pre flop super agressive AI
def preFlopImranAI(app, hand, numPlayersLeft, betSize = 1/100):
    ...
    if numPlayersLeft == 7:
        ...
        return preFlop7OthersImranAIAction(handOdds, betSize), handOdds
        ...
        return preFlop6OthersImranAIAction(handOdds, betSize), handOdds
        ...
        return preFlop5OthersImranAIAction(handOdds, betSize), handOdds
        ...
        return preFlop4OthersImranAIAction(handOdds, betSize), handOdds
        ...
        return preFlop3OthersImranAIAction(handOdds, betSize), handOdds
        ...
        return preFlop2OthersImranAIAction(handOdds, betSize), handOdds
        ...
        return preFlop1OtherImranAIAction(handOdds, betSize), handOdds

# pre flop with 7 others
def preFlop7OthersImranAIAction(odds, betSize):
    action = ""
...
def preFlop6OthersImranAIAction(odds, betSize):
    action = ""
...
def preFlop5OthersImranAIAction(odds, betSize):
    action = ""
...
def preFlop4OthersImranAIAction(odds, betSize):
    action = ""
...
def preFlop3OthersImranAIAction(odds, betSize):
    action = ""
...
def preFlop2OthersImranAIAction(odds, betSize):
    action = ""
...
def preFlop1OtherImranAIAction(odds, betSize):
    action = ""
...
# print(preFlopImranAI(["AC","TH"], 5, .35))

# runs pre turn super agressive AI
def preTurnImranAI(app, hand, board, numPlayersLeft, callSize = 0):

    if numPlayersLeft == 7:
        handOdds = postFlopOdds(hand, board, 7, 1000)
        handOdds += pokerMLOddsAdjustment(app)
        return preTurn7OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 6:
        handOdds = postFlopOdds(hand, board, 6, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 1.4
        return preTurn6OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 5:
        handOdds = postFlopOdds(hand, board, 5, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 1.8
        return preTurn5OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 4:
        handOdds = postFlopOdds(hand, board, 4, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 2.2
        return preTurn4OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 3:
        handOdds = postFlopOdds(hand, board, 3, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 2.6
        return preTurn3OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 2:
        handOdds = postFlopOdds(hand, board, 2, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 3
        return preTurn2OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 1:
        handOdds = postFlopOdds(hand, board, 1, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 3.5
        return preTurn1OtherImranAIAction(handOdds, callSize)
    
# pre turn with 7 others
def preTurn7OthersImranAIAction(handOdds, callSize):
    action = ""
    ...
    if callSize == 0:
        ...
        if handOdds <= .15:
            action = ("Fold")
        elif .15 < handOdds <= .225:
            randNum = random.randint(1, 100)
            if randNum <= 85:
                action = ("Call")
            else:
                action = ("Fold")
        elif .225 < handOdds <= .325:
            action = ("Call")
        elif .325 < handOdds <= .475:
            randNum = random.randint(1, 100)
            if randNum >= 50:
...
def preTurn6OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preTurn5OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preTurn4OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preTurn3OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preTurn2OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preTurn1OtherImranAIAction(handOdds, callSize):
    action = ""
...
# print(preTurnImranAI(["AD","AS"], ["7H","3D","7D"], 4, 0.1))

# runs pre river super agressive AI
def preRiverImranAI(app, hand, board, numPlayersLeft, callSize = 0):

    if numPlayersLeft == 7:
        handOdds = postTurnOdds(hand, board, 7, 1000)
        handOdds += pokerMLOddsAdjustment(app)
        return preRiver7OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 6:
        handOdds = postTurnOdds(hand, board, 6, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 1.6
        return preRiver6OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 5:
        handOdds = postTurnOdds(hand, board, 5, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 2.2
        return preRiver5OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 4:
        handOdds = postTurnOdds(hand, board, 4, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 2.8
        return preRiver4OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 3:
        handOdds = postTurnOdds(hand, board, 3, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 3.4
        return preRiver3OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 2:
        handOdds = postTurnOdds(hand, board, 2, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 4
        return preRiver2OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 1:
        handOdds = postTurnOdds(hand, board, 1, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 4.5
        return preRiver1OtherImranAIAction(handOdds, callSize)

# pre river with 7 others
def preRiver7OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preRiver6OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preRiver5OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preRiver4OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preRiver3OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preRiver2OthersImranAIAction(handOdds, callSize):
    action = ""
...
def preRiver1OtherImranAIAction(handOdds, callSize):
    action = ""
...
# print(preRiverImranAI(["AD","AS"], ["7H","3D","7D","7S"], 4, 0.1))

# runs post river super agressive AI
def postRiverImranAI(app, hand, board, numPlayersLeft, callSize = 0):

    if numPlayersLeft == 7:
        handOdds = postRiverOdds(hand, board, 7, 1000)
        handOdds += pokerMLOddsAdjustment(app)
        return postRiver7OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 6:
        handOdds = postRiverOdds(hand, board, 6, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 1.75
        return postRiver6OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 5:
        handOdds = postRiverOdds(hand, board, 5, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 2.5
        return postRiver5OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 4:
        handOdds = postRiverOdds(hand, board, 4, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 3.25
        return postRiver4OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 3:
        handOdds = postRiverOdds(hand, board, 3, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 4
        return postRiver3OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 2:
        handOdds = postRiverOdds(hand, board, 2, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 4.75
        return postRiver2OthersImranAIAction(handOdds, callSize)

    elif numPlayersLeft == 1:
        handOdds = postRiverOdds(hand, board, 1, 1000)
        handOdds += pokerMLOddsAdjustment(app) * 5.5
        return postRiver1OtherImranAIAction(handOdds, callSize)

# post river with 7 others
def postRiver7OthersImranAIAction(handOdds, callSize):
    action = ""
...
def postRiver6OthersImranAIAction(handOdds, callSize):
    action = ""
...
def postRiver5OthersImranAIAction(handOdds, callSize):
    action = ""
...
def postRiver4OthersImranAIAction(handOdds, callSize):
    action = ""
...
def postRiver3OthersImranAIAction(handOdds, callSize):
    action = ""
...
def postRiver2OthersImranAIAction(handOdds, callSize):
    action = ""
...
def postRiver1OtherImranAIAction(handOdds, callSize):
    action = ""
...
# print(postRiverImranAI(["AD","AS"], ["7H","3D","7D","7S"], 4, 0.1))